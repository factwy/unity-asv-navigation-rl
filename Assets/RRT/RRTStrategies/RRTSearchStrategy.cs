...
namespace RRT.RRTStrategies
{
    ...
    public abstract class RRTSearchStrategy
    {
        ...
        {
            // Raycast 대신 SphereCast를 사용하여, Config에 저장된 AgentRadius 크기의 구체로 충돌을 검사합니다.
            var isColliding = Physics.SphereCast(posStart, Config.AgentRadius, direction, out var hit, directionMagnitude,
                Config.ObstacleLayerMask);
            
            // 만약 충돌했더라도, 그 대상이 최종 목표(Target)라면 장애물로 취급하지 않습니다.
